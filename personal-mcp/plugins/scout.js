...
async function fetchRentEstimate(address, bedrooms) {
  const key = process.env.RENTCAST_API_KEY;
  if (!key) return null;
  try {
...
// ── Deal scoring ──────────────────────────────────────────────
function dealScore({ capRate, coc, dscr, grm, pricePerDoor }) {
  let score = 0;
  // Cap rate (0-30 pts)
...
const tools = [
  {
    ...
    async handler(p) {
      ...
      if (process.env.RENTCAST_API_KEY) {
        rentcastData = await fetchRentcast(p.address);
        if (rentcastData) {
          enriched = {
        ...
        if (!p.monthly_rent && !rentcastData?.rentPrice) {
          const est = await fetchRentEstimate(p.address, p.bedrooms || 3);
      ...
      const tax        = p.monthly_taxes ?? p.asking_price * 0.01 / 12;  // 1% annual estimate
      ...
      const score   = dealScore({ capRate, coc, dscr, grm, pricePerDoor });
...

  {
    ...
    async handler(p) {
      ...
      // Return on investment: if cash left = 0, return is "infinite"
      const coc = cashLeft > 0 ? (annualCF / cashLeft * 100) : null;

      // Holding cost estimate (no rent during rehab, interest on hard money ~10%)
...

  {
    ...
    async handler(p) {
      ...
      const downPct      = p.down_payment_pct ?? 5;   // FHA allows 3.5-5% on 2-4 unit
      const rate         = p.loan_rate ?? 7.0;
      const term         = 30;
      const vacancy      = p.vacancy_rate ?? 8;
      ...
      // Equity building per month (principal paydown)
...

  {
    ...
    async handler(p) {
      ...
      // Your MAO = buyer max purchase - your assignment fee
      const mao = buyerMaxPurchase - fee;

      // Classic 70% rule check
...

  {
    ...
    async handler({ address, state, bedrooms = 2 }) {
      const results = {};
      ...
      if (hudKey) {
        try {
          ...
          const data = await res.json();
          // Extract FMR for the bedroom size we want
          results.hud_fmr = {
      ...
      if (address && process.env.RENTCAST_API_KEY) {
        const est = await fetchRentEstimate(address, bedrooms);
        if (est) {
          results.rentcast = {
            address,
            bedrooms,
...

  {
    ...
    async handler({ sort_by = 'deal_score' }) {
      const db = getDb();
      const rows = db.prepare("SELECT * FROM properties WHERE status = 'pipeline' ORDER BY created_at DESC").all();
...

  {
    ...
    async handler({ addresses = [], ids = [] }) {
      const db = getDb();
      let rows = [];

      if (ids.length) {
        rows = ids.map(id => db.prepare("SELECT * FROM properties WHERE id = ?").get(id)).filter(Boolean);
      } else if (addresses.length) {
        rows = addresses.map(addr =>
          db.prepare("SELECT * FROM properties WHERE address LIKE ? AND status = 'pipeline'").get(`%${addr}%`)
        ).filter(Boolean);
      } else {
        rows = db.prepare("SELECT * FROM properties WHERE status = 'pipeline' ORDER BY created_at DESC LIMIT 5").all();
      }
