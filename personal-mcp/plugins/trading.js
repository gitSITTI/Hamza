const { getDb, encrypt } = require('../db');
...
const tools = [
  ...
  // ── Tool 1: save_income_position ──────────────────────────────────────────
  {
    ...
    async handler({ ticker, shares, cost_basis_per_share, monthly_dist_per_share, annual_yield_pct, account_type = 'taxable', broker, notes }) {
      const db = getDb();
      const sym = ticker.toUpperCase();
      const etfProfile = INCOME_ETFS[sym] || null;

      // Fill yield data from reference table if not provided
      ...
      // Upsert account
      let account = db.prepare('SELECT id FROM accounts WHERE type = ? AND institution = ?').get(
        account_type, broker || 'Unknown'
      );
      if (!account) {
        const aid = uuid();
        db.prepare('INSERT INTO accounts (id, type, institution, nickname) VALUES (?, ?, ?, ?)').run(
          aid, account_type, broker || 'Unknown', `${broker || 'Unknown'} ${account_type}`
        );
        account = { id: aid };
      }

      // Store extra metadata as JSON in the notes column
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(id, account.id, sym, 'income_etf', shares, encrypt(String(totalCostBasis)), notesJson);
  ...
  // ── Tool 2: log_options_trade ─────────────────────────────────────────────
  {
    ...
    async handler({ ticker, strategy, contract_type, underlying, strike, expiry, contracts = 1, premium_collected, open_date, account_type, broker, notes }) {
      const db  = getDb();
      const sym = ticker.toUpperCase();
      ...
      if (broker || account_type) {
        account = db.prepare('SELECT id FROM accounts WHERE type = ? AND institution = ?').get(
          account_type || 'margin', broker || 'Unknown'
        );
        if (!account) {
          const aid = uuid();
          db.prepare('INSERT INTO accounts (id, type, institution, nickname) VALUES (?, ?, ?, ?)').run(
            aid, account_type || 'margin', broker || 'Unknown', `${broker || 'Unknown'} ${account_type || 'margin'}`
          );
          account = { id: aid };
        }
      }
  ...
  // ── Tool 3: close_options_trade ───────────────────────────────────────────
  {
    ...
    async handler({ trade_id, close_reason, premium_paid = 0, close_date, notes }) {
      const db = getDb();
      const trade = db.prepare('SELECT * FROM options_trades WHERE id = ?').get(trade_id);
      ...
      db.prepare(`
        ...
        WHERE id = ?
      `).run(status, closeDateStr, premium_paid, net_pnl, notes ?? trade.notes, trade_id);
  ...
  // ── Tool 4: get_options_pnl ───────────────────────────────────────────────
  {
    ...
    async handler({ period = 'month', year, month, strategy }) {
      const db   = getDb();
      ...
      const trades = db.prepare(`
        ...
        WHERE 1=1 ${dateFilter} ${stratFilter}
  ...
  // ── Tool 5: log_distribution ──────────────────────────────────────────────
  {
    ...
    async handler({ ticker, distribution_date, per_share_amount, shares, reinvested = false, account_type, broker }) {
      const db  = getDb();
      const sym = ticker.toUpperCase();
      const distDate = distribution_date ?? todayStr();

      // Look up shares from saved position if not provided
      let effectiveShares = shares;
      if (!effectiveShares) {
        const pos = db.prepare(`SELECT quantity FROM market_positions WHERE ticker = ? AND asset_type = 'income_etf' ORDER BY updated_at DESC LIMIT 1`).get(sym);
      ...
      // YTD total for this ticker
      const yearStr = distDate.slice(0, 4);
      const ytd = db.prepare(`
        SELECT SUM(total_received) as total FROM income_distributions
        WHERE ticker = ? AND distribution_date LIKE ?
      `).get(sym, `${yearStr}%`);
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(id, sym, distDate, per_share_amount, effectiveShares, total_received, reinvested ? 1 : 0);
  ...
  // ── Tool 6: get_monthly_income ────────────────────────────────────────────
  {
    ...
    async handler({ year, month }) {
      ...
      // ETF distributions for the month
      const distRows = db.prepare(`
        SELECT ticker, SUM(total_received) as amount
        FROM income_distributions
        WHERE distribution_date LIKE ?
        GROUP BY ticker
      `).all(prefix);
      const etf_distributions = +distRows.reduce((s, r) => s + r.amount, 0).toFixed(2);

      // Closed options trades with positive P&L
      const closedTrades = db.prepare(`
        SELECT ticker, SUM(net_pnl) as amount
        FROM options_trades
        WHERE close_date LIKE ? AND status IN ('expired', 'closed') AND net_pnl > 0
        GROUP BY ticker
      `).all(prefix);
      const options_realized = +closedTrades.reduce((s, r) => s + r.amount, 0).toFixed(2);

      // Open options trades (premium collected, unrealized)
      const openTrades = db.prepare(`
        SELECT ticker, SUM(premium_collected) as amount
        FROM options_trades
        WHERE open_date LIKE ? AND status = 'open'
        GROUP BY ticker
      `).all(prefix);
  ...
  // ── Tool 7: living_off_trading ────────────────────────────────────────────
  {
    ...
    async handler({ monthly_expenses, target_monthly_income, current_portfolio_value }) {
      const db = getDb();

      // Pull monthly expenses from obligations if not provided
      let expenseBase = monthly_expenses;
      if (!expenseBase) {
        const obligs = db.prepare(`SELECT SUM(monthly_amount) as total FROM obligations`).get();
        expenseBase = obligs?.total ?? 0;
      }
      const targetIncome = target_monthly_income ?? expenseBase;

      // Last 3 months trailing income
      const now   = new Date();
      const months = [];
      for (let i = 0; i < 3; i++) {
      ...
      for (const prefix of months) {
        const dist = db.prepare(`SELECT SUM(total_received) as t FROM income_distributions WHERE distribution_date LIKE ?`).get(prefix);
        const opts = db.prepare(`SELECT SUM(net_pnl) as t FROM options_trades WHERE close_date LIKE ? AND status IN ('expired','closed') AND net_pnl > 0`).get(prefix);
      ...
      // Portfolio needed at different yields
  ...
  // ── Tool 8: broker_snapshot ───────────────────────────────────────────────
  {
    ...
    async handler({ broker, account_type, equity, cash, margin_used, margin_available, buying_power, positions_value, day_pnl, unrealized_pnl, realized_pnl_ytd, snapshot_date, notes }) {
      const db = getDb();
  ...
  // ── Tool 9: get_broker_summary ────────────────────────────────────────────
  {
    ...
    async handler() {
      ...
      // Latest snapshot per broker+account_type combination
  ...
  // ── Tool 10: income_etf_screener ──────────────────────────────────────────
  {
    ...
    async handler({ invested_amount = 10000, yield_min_pct = 0, freq_filter = 'all' }) {
      ...
      const etfs = filtered.map(([ticker, v]) => {
        ...
        // Margin spread analysis — net spread vs each broker
