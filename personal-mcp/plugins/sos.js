...
async function searchMoSos(name) {
  ...
  try {
    ...
    // If JSON, parse it. If HTML, return helpful fallback.
    try {
      const json = JSON.parse(text);
      return { ok: true, data: json };
    } catch {
      // HTML response — parse not available
...
const tools = [
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        name: { type: 'string', description: 'Business name or partial name to search' }
      },
      required: ['name']
    },
    async handler({ name }) {
...

  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        name: { type: 'string', description: 'Business name to look up' }
      },
      required: ['name']
    },
    async handler({ name }) {
...

  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        name: { type: 'string', description: 'Business name to search across both states' }
      },
      required: ['name']
    },
    async handler({ name }) {
      const [mo, va] = await Promise.allSettled([
        tools.find(t => t.name === 'lookup_entity_mo').handler({ name }),
        tools.find(t => t.name === 'lookup_entity_va').handler({ name })
      ]);
      return {
        query: name,
...

  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        state: { type: 'string', description: 'Filter by state code (optional)' }
      }
    },
    async handler({ state } = {}) {
      const db = getDb();
      let rows;
      if (state) {
        rows = db.prepare('SELECT * FROM entities WHERE state = ? ORDER BY created_at DESC').all(state);
      } else {
        rows = db.prepare('SELECT * FROM entities ORDER BY created_at DESC').all();
      }
...

  {
    ...
    async handler(args) {
      ...
      let entity = null;
      if (args.entity_id) {
        entity = db.prepare('SELECT * FROM entities WHERE id = ?').get(args.entity_id);
      } else if (args.name) {
        entity = db.prepare('SELECT * FROM entities WHERE name LIKE ?').get(`%${args.name}%`);
      }
