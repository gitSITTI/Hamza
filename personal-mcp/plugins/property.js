...
const tools = [
  {
    ...
    description: 'Look up a property by address. Fetches value, rent estimate, and assessor data from Rentcast. Returns raw data for further analysis.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    async handler({ address }) {
      const rc = await fetchRentcast(address);
      if (!rc) return { message: 'No Rentcast data (check RENTCAST_API_KEY or address)', address };
      return {
        address:       rc.formattedAddress,
        property_type: rc.propertyType,
        bedrooms:      rc.bedrooms,
        bathrooms:     rc.bathrooms,
        sqft:          rc.squareFootage,
        year_built:    rc.yearBuilt,
        lot_size:      rc.lotSize,
        zestimate:     rc.price,
        rent_estimate: rc.rentPrice,
        last_sold_price: rc.lastSalePrice,
        last_sold_date:  rc.lastSaleDate,
        raw: rc
      };
    }
  },

  {
    ...
    description: 'Full investment analysis for a property. Returns cap rate, cash-on-cash, DSCR, GRM, 10-year IRR, and a go/no-go recommendation.',
    inputSchema: {
      type: 'object',
      properties: {
        purchase_price:    { type: 'number' },
        down_payment_pct:  { type: 'number', description: 'e.g. 20 for 20%' },
        loan_rate:         { type: 'number', description: 'Annual interest rate, e.g. 7.0' },
        loan_term:         { type: 'number', description: 'Years, e.g. 30' },
        monthly_rent:      { type: 'number' },
        vacancy_rate:      { type: 'number', description: 'e.g. 8 for 8%' },
        monthly_tax:       { type: 'number' },
        monthly_insurance: { type: 'number' },
        capex_pct:         { type: 'number', description: 'CapEx as % of rent, e.g. 10' },
        mgmt_pct:          { type: 'number', description: 'Property mgmt fee % of rent, e.g. 8' },
        appreciation_rate: { type: 'number', description: 'Annual appreciation %, e.g. 3' }
      },
    ...
    async handler(p) {
      ...
      const appRate    = p.appreciation_rate ?? 3;
      ...
      const tax        = p.monthly_tax       ?? 0;
      const ins        = p.monthly_insurance ?? 0;
      ...
      // 10-year IRR: initial investment = -down, annual cash flows, year 10 exit
      const flows = [-down];
      for (let y = 1; y <= 10; y++) {
        const annualCF = cashflow * Math.pow(1.02, y); // 2% rent growth
        if (y < 10) { flows.push(annualCF); }
          ...
          const exitEquity   = exitPrice - remainBal;
          flows.push(annualCF + exitEquity);
        }
      }
      ...
      return {
        ...
        metrics: {
          ...
          cash_flow_annual: +cashflow.toFixed(0),
          cash_flow_monthly: +(cashflow / 12).toFixed(0),
...

  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        address:          { type: 'string' },
        type:             { type: 'string', description: 'sfh | multifamily | commercial | land' },
        status:           { type: 'string', description: 'owned | pipeline | sold' },
        purchase_price:   { type: 'number' },
        current_value:    { type: 'number' },
        mortgage_balance: { type: 'number' },
        mortgage_rate:    { type: 'number' },
        mortgage_term:    { type: 'number' },
        monthly_rent:     { type: 'number' },
    ...
    async handler(args) {
      ...
      await indexDoc({
        id: `property-${id}`,
        category: 'property',
        title: args.nickname || args.address,
...

  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        status: { type: 'string', description: 'Filter by: owned | pipeline | sold (default: owned)' }
      }
    },
    async handler({ status = 'owned' }) {
      const db = getDb();
      const rows = db.prepare('SELECT * FROM properties WHERE status = ? ORDER BY created_at').all(status);
      const summary = rows.map(p => ({
        ...p,
        equity: (p.current_value || p.purchase_price || 0) - (p.mortgage_balance || 0),
        monthly_cashflow: p.monthly_rent ? p.monthly_rent - monthlyPayment(
          p.mortgage_balance || 0, p.mortgage_rate || 7, p.mortgage_term || 30) : null
      }));
...

  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        monthly_expenses:{ type: 'number', description: 'Tax + insurance + capex combined' }
      },
      required: ['purchase_price', 'monthly_rent', 'loan_amount']
    },
    async handler(p) {
      function cashflow(rent, rate) {
