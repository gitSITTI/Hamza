// Projects, goals, gap analysis, session context — Worker/D1 port of plugins/projects.js
...
export default [
  {
    ...
    async handler({ status }, { db }) {
      const rows = status
        ? await db.all('SELECT * FROM projects WHERE status = ? ORDER BY updated_at DESC', status)
        : await db.all('SELECT * FROM projects ORDER BY status, updated_at DESC');
  ...
  },
  {
    ...
    async handler({ name_or_id }, { db }) {
      const proj = await db.get('SELECT * FROM projects WHERE id = ? OR name LIKE ?', name_or_id, `%${name_or_id}%`);
  ...
  },
  {
    ...
    async handler(args, ctx) {
      ...
      if (args.id) {
        await db.run(
          `UPDATE projects SET name=?, description=?, status=?, stack=?, repo=?, branch=?,
            progress_pct=?, context_block=?, notes=?, updated_at=datetime('now') WHERE id=?`,
      ...
      const id = uid();
      await db.run(
        `INSERT INTO projects (id, name, description, status, stack, repo, branch, progress_pct, context_block, notes)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ...
  },
  {
    ...
    async handler(_args, { db }) {
      ...
      const projects = await db.all('SELECT name, status, progress_pct, context_block, repo, branch FROM projects WHERE status = ?', 'active');
      const snap = await db.get('SELECT label, net_worth, created_at FROM snapshots ORDER BY created_at DESC LIMIT 1');
      const obls = await db.all('SELECT type, counterparty, monthly_amount FROM obligations');
      const props = await db.all('SELECT nickname, address, status, current_value, mortgage_balance FROM properties WHERE status = ?', 'owned');
      const goals = await db.all('SELECT title, target_value, current_value, target_date FROM goals ORDER BY target_date');
  ...
  },
  {
    ...
    async handler(args, { db }) {
      const id = uid();
      await db.run(
        'INSERT INTO goals (id, title, category, target_value, current_value, target_date, notes) VALUES (?, ?, ?, ?, ?, ?, ?)',
  ...
  },
  {
    ...
    inputSchema: { type: 'object', properties: {} },
    async handler(_args, { db }) {
      const goals = await db.all('SELECT * FROM goals ORDER BY target_date');
      const snap = await db.get('SELECT net_worth FROM snapshots ORDER BY created_at DESC LIMIT 1');
