// Document vault, tax summary, extract_tax_data — Worker/D1 port of plugins/documents.js
...
export default [
  {
    ...
    async handler(args, ctx) {
      ...
      await db.run(
        `INSERT INTO documents (id, category, year, filename, parsed_data, summary)
          VALUES (?, ?, ?, ?, ?, ?)`,
  ...
  },
  {
    ...
    async handler({ category, year } = {}, { db }) {
  ...
  },
  {
    ...
    async handler({ id, filename } = {}, { db }) {
      let doc;
      if (id) {
        doc = await db.get('SELECT * FROM documents WHERE id = ?', id);
      } else if (filename) {
        doc = await db.get('SELECT * FROM documents WHERE filename LIKE ?', `%${filename}%`);
      }
  ...
  },
  {
    ...
    async handler({ keyword, category, year_from, year_to } = {}, { db }) {
      let query = 'SELECT id, category, year, filename, summary, created_at FROM documents WHERE 1=1';
      const params = [];
      if (category) { query += ' AND category = ?'; params.push(category); }
  ...
  },
  {
    ...
    async handler(_args, { db }) {
      const rows = await db.all('SELECT * FROM documents WHERE category = ? ORDER BY year DESC', 'tax');
