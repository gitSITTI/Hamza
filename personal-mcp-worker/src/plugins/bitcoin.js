// Bitcoin models: S2F, power law, Saylor thesis, scenarios — Worker/D1 port of plugins/bitcoin.js
...
async function getLiveBtcPrice(ctx) {
  const { db, env } = ctx;
...
// ── Shared math helpers ────────────────────────────────────────
const CIRCULATING_SUPPLY_EST = 19_718_400;
const ANNUAL_FLOW = 3.125 * 144 * 365;
...
export default [
  {
    ...
    async handler(_args, ctx) {
      ...
      const actualPrice = await getLiveBtcPrice(ctx);
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        target_date: { type: 'string', description: 'Optional YYYY-MM-DD date to project the corridor. Defaults to today.' },
      },
    },
    async handler({ target_date } = {}, ctx) {
      ...
      const actualPrice = target_date ? null : await getLiveBtcPrice(ctx);
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        years: { type: 'number', description: 'Projection horizon in years (default 10)' },
        capital_amount: { type: 'number', description: 'USD capital to analyze (default 100000)' },
        btc_cagr_assumption: { type: 'number', description: 'Annual BTC growth % assumption (default 50 = conservative)' },
      },
    },
    async handler({ years = 10, capital_amount = 100_000, btc_cagr_assumption = 50 } = {}) {
      const m2DecayRate = 0.07;
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        amount_usd: { type: 'number', description: "USD amount you plan to invest at today's price (optional)" },
        current_btc_holdings: { type: 'number', description: 'BTC you already hold (optional) — used to project portfolio value' },
      },
    },
    async handler({ amount_usd, current_btc_holdings } = {}, ctx) {
      const currentPrice = await getLiveBtcPrice(ctx);
